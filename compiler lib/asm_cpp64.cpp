...
static inline bool strip_comments(std::string& line) {
    // ignore ligne vide dès le début
    ...
    // couper sur ; seulement s'il n'est pas entre guillemets
    size_t i = 0;
    bool inside_quotes = false;
    while (i < line.size()) {
        if (line[i] == '"') {
            // vérifier backslashes avant guillemet
            size_t bs = 0;
            size_t j = i;
            while (j > 0 && line[j-1] == '\\') { bs++; j--; }
            if (bs % 2 == 0) inside_quotes = !inside_quotes;
        }
...
//pour l'encodage utf-8 du fichier à lire
std::string read_file_auto_encoding(FILE* f) {
    if (!f) return {};
    ...
    if (sz >= 3 &&
        ...
        return std::string((char*)buf.data() + 3, sz - 3);
    }
    ...
    if (sz >= 2 &&
        ...
        int needed = WideCharToMultiByte(CP_UTF8, 0, w, wlen, nullptr, 0, nullptr, nullptr);
        std::string out(needed, 0);
        WideCharToMultiByte(CP_UTF8, 0, w, wlen, out.data(), needed, nullptr, nullptr);
        return out;
    }
    ...
    // ANSI / ACP -> UTF-8
    // =========================
    int wlen = MultiByteToWideChar(CP_ACP, 0, (char*)buf.data(), sz, nullptr, 0);
    if (wlen <= 0) return {};

    std::wstring w(wlen, 0);
    MultiByteToWideChar(CP_ACP, 0, (char*)buf.data(), sz, w.data(), wlen);

    int needed = WideCharToMultiByte(CP_UTF8, 0, w.data(), wlen, nullptr, 0, nullptr, nullptr);
    std::string out(needed, 0);
    WideCharToMultiByte(CP_UTF8, 0, w.data(), wlen, out.data(), needed, nullptr, nullptr);
...
.positive:
    ...
    mov rcx, rax
    mov rdx, rdi
    lea rax, [rip+@asm_addr_int_ascii]
    call rax
...
//debut edata
std::string make_ascii_export_name(const std::string& input) {
    std::string out;
    for (unsigned char c : input) {
        // ASCII imprimable
        if (c < 32 || c > 126)
            continue;
        // caractères interdits PE/export
        switch (c) {
            case ' ':
            case '\t':
            case '\r':
...
bool parse_edata_section(const SEP_SECTION& section, GESTION& g) {
    for (const auto& line : section.lines) {
        ...
        if (cmd == "local") {
            if (tokens.size() < 2 || tokens.size() > 3) {
                std::cerr
                    << "edata local invalide : "
                    << line
                    << std::endl;
                continue;
            }
            std::string name = make_ascii_export_name(parse_escaped_string(unquote(tokens[1]))); //((tokens.size()==2) ? 1:2)

            if (name.empty()) {
                std::cerr
                    << "edata local nom vide : "
                    << line
                    << std::endl;
                continue;
            }
            if (tokens.size()==2) g.edata_add_export(name);
            else {
                std::string localname = make_ascii_export_name(parse_escaped_string(unquote(tokens[2])));
                g.edata_add_export(name,0xFFFFFFFF, localname);
            }
        }
        else if (cmd == "extern") {
            if (tokens.size() != 3) {
                std::cerr
                    << "edata extern invalide : "
                    << line
                    << std::endl;
                continue;
            }
            ...
            // -------- nom export --------
            std::string raw_name = parse_escaped_string(unquote(tokens[1]));

            // -------- lien --------

            std::string raw_link = parse_escaped_string(unquote(tokens[2]));

            std::string name = make_ascii_export_name(raw_name);

            std::string link = make_ascii_export_name(raw_link);
            ...
            g.edata_add_reexport(name,link);
        }
        else {
            std::cerr << "Commande edata inconnue : " << cmd << std::endl;
        }
    }
    return true;
}
...
bool parse_asm_file_sections(const std::string& path, std::vector<SEP_SECTION>& out_sections, bool in_include_file = false)
{
    ...
    if (read_bytes != file_size) {
        std::cerr << "Erreur : lecture du fichier échouée !" << std::endl;
        return false;
    }*/
    ...
    std::string content = read_file_auto_encoding(f);
    fclose(f);
    if (content.empty()) {
        std::cerr << "Erreur : lecture du fichier échouée !" << std::endl;
        return false;
    ...
    while (start < content.size()) {
        ...
        if (!current_section) {
            if (!starts_with(line, "ASM64CPP") && !read_first_line) {
                std::cerr << "WARNING : le fichier " << path << " ne commence pas par ASM64CPP !" << std::endl;
                read_first_line=true;
                continue;
        ...
        current_section->lines.emplace_back(std::move(line));
        read_first_line=true;
    }
    return true;
}

//pile de fonction introuvable de la lib
static std::unordered_set<std::string> unresolved_names;
//resolver de la lib
void setup_idata_resolver(GESTION& g, const IDATA_CONTEXT& ctx)
{
    g.set_text_external_resolver([&g, &ctx](const std::string& name) -> u64 {
        ...
        if (v != 0) {
            unresolved_names.erase(name); // retirer liste introuvable
            return v;
        }
        ...
        if (unresolved_names.find(name) == unresolved_names.end()) {
            unresolved_names.insert(name); // Ajouter liste introuvables
        }
        return 0;
    });
}
...
int main(int argc, char* argv[])
{
    ...
    g.edata_set_dll_name(output_exe); //défini le nom la dll dans la section edata au nom de l'exe par défaut
    ...
    for (const auto& sec : sections) {
        ...
        //edata
        if (sec.name == ".edata") {
            if (!parse_edata_section(sec, g)) {
                std::cerr << "erreur fatale edata !"<< std::endl;
                return 1;
            }
        }
    ...
    // ! WARNING !
    //text en priorité sinon risque de crach car cette section est créer par défaut dans le constructeur et se retrouve donc toujours en premier (si on ne la met pas en premier on ne peut exécuté qu'un code avec peut de fonction inférieur à SECTION_ALIGNEMENT)
    for (const auto& sec : sections) {
        if (sec.name == ".text") {
    ...
    for (const auto& sec : sections) {
        if (sec.name == ".data" || sec.name == ".edata" || sec.name == ".idata" || sec.name == ".rsrc") {
            g.push_section(sec.name);
        }
    }
    ...
    //affiche les noms introuvables de data idata et des fonction text ...
    if (!unresolved_names.empty()) {
        std::cerr << "Erreur fatale de resolution de nom :\nlistes des noms utilisés dans le code introuvable :" << std::endl;
        for (const auto& unresolved_name : unresolved_names) {
            std::cout << "- $" << unresolved_name << std::endl;
        }
        return 1;
    }

    if (g.edata_have_unresolved_export_rva()) {
        std::cerr << "Warning : résolution de certains symboles d'exportation dans edata ont échoué !\ncertains liens en utilisation dll seront impossibles !" << std::endl;
    }
