...
static bool parse_bool(const std::string& s)
{
    ...
    std::cerr << "bool invalide : " << s << std::endl;
    return false;
}
...
int main(int argc, char* argv[])
{
    if (argc < 3) {
        std::cout << "Usage:\n";
        std::cout << "  " << argv[0] << " <source.asm> <output.exe> [--see_all_exception] [--cmd]\n";
        return 1;
    }
    ...
    for (const auto& sec : sections) {
        if (sec.name == ".data") {
            if (!parse_data_section(sec, g)) {
                std::cout << "erreur fatale data !"<< std::endl;
                return 1;
            }
            break;
        }
    ...
    for (const auto& sec : sections) {
        if (sec.name == ".text") {
            if (!parse_text_section(sec, g)) {
                std::cout << "erreur fatale text !"<< std::endl;
                return 1;
            }
            break;
        }
    ...
    for (const auto& sec : sections) {
        if (sec.name == ".rsrc") {
            if (!parse_rsrc_section(sec, g)) {
                std::cout << "erreur fatale rsrc !" << std::endl;
                return 1;
            }
            break;
        }
    ...
    if (entry_va == 0) {
        std::cout << "Erreur : entry point non résolu ou pas de fonction main (obligatoire)" << std::endl;
        return 1;
    }
    g.set_addr_entry_point(entry_va);
    g.buid_exe();

    std::cout << "[+] Configuration OK, prêt à parser : " << asm_file << std::endl;
