...
Ce code est optimiser dans les controls  grâce à la class GESTION on peut tout faire de façon simplifier
ATTENTION la fonction push_section il faut toujours pousser la section .text en premier sinon les calculs sont faussé car le constructeur construit la section .text automatiquement.
...
u32 compute_pe_checksum(const std::vector<u8>& file, size_t checksum_offset)
{
    u64 sum = 0;
    const u32* data = (const u32*)file.data();
...
public:
    ...
    vector<u8> build(u32 entry_rva, bool with_console=true, bool is_dll = false) {
        entryRVA = entry_rva;
        layout();
        ...
        *(u32*)&hdr[0x3C] = e_lfanew;
        ...
        u16 characteristics_app =
            0x0002 | // EXECUTABLE_IMAGE
            0x0020 | // LARGE_ADDRESS_AWARE
            0x2000;  // DLL
        if (is_dll) push_u16(characteristics_app); // Characteristics  exe seulement -> 0x0022
        else push_u16(0x0022);
        ...
        u32 checksum = compute_pe_checksum(file, off_checksum);
        *(u32*)&file[off_checksum] = checksum;
...
private:
    ...
    std::vector<u8> content;
    std::vector<ReExportFunction> reexports;
    std::vector<FinalExport> final_exports;

    std::string dll_name = "mydll.dll";
...
public:
    ...
    void add_export(const std::string& name, u32 rva = 0xFFFFFFFF, const std::string& localname = "") {
        // remplace si existe déjà
        for (auto& e : exports) {
            if (e.name == name) {
                e.rva = rva;
                if (localname.size()) e.localname = localname;
                else e.localname = name;
                return;
            }
        }
        // si existe en reexport -> remplace
        for (size_t i = 0; i < reexports.size(); i++) {
            if (reexports[i].name == name) {
                reexports.erase(reexports.begin() + i);
                break;
            }
        }
        exports.push_back({name, (localname.size() ? localname: name), rva});
    }
    ...
    void add_reexport(const std::string& name, const std::string& link) {
        // remplace si existe déjà
        for (auto& e : reexports) {
            if (e.name == name) {
                e.link = link;
                return;
            }
        }
        // supprime export normal si existe
        ...
        reexports.push_back({name, link});
    }

    bool have_unresolved_export_rva() {
        for (auto& e : exports) {
            if (e.rva == 0xFFFFFFFF) {
                return true;
            }
    ...
    void prepare(u32 edata_rva) {
        content.clear();
        final_exports.clear();

        for (auto& e : exports) {
            final_exports.push_back({
                e.name,
                false,
                e.rva,
                0,
        ...
        std::sort(final_exports.begin(), final_exports.end(), [](const FinalExport& a, const FinalExport& b) {
            return a.name < b.name;
        });

        u32 export_count = static_cast<u32>(final_exports.size());
        ...
        u32 export_dir_offset = 0;

        u32 eat_offset = export_dir_offset + 40;

        u32 ent_offset = eat_offset + export_count * 4;

        u32 eot_offset = ent_offset + export_count * 4;

        u32 dll_name_offset = eot_offset + export_count * 2;

        u32 names_offset = dll_name_offset + static_cast<u32>(dll_name.size() + 1);

        // =========================
        // calcul RVA noms exports
        // =========================

        std::vector<u32> name_rvas;

        u32 current_name_offset = names_offset;

        for (auto& e : final_exports) {
            name_rvas.push_back(
                edata_rva + current_name_offset
            );

            current_name_offset +=
                static_cast<u32>(e.name.size() + 1);
        }
        ...
        //std::vector<u32> forward_rvas;

        u32 current_forward_offset = current_name_offset;

        for (auto& e : final_exports) {
            if (e.is_forward) {
                e.forward_rva = edata_rva + current_forward_offset;

                current_forward_offset +=
                static_cast<u32>(e.link.size() + 1);
            }
        }
        ...
        push_u32(edata_rva + dll_name_offset);

        push_u32(1);

        push_u32(export_count);
        push_u32(export_count);

        push_u32(edata_rva + eat_offset);
        push_u32(edata_rva + ent_offset);
        push_u32(edata_rva + eot_offset);
    ...
    std::vector<u8> get_content() const {
        return content;
    }

    u32 get_size() const {
        return static_cast<u32>(content.size());
    }
};
...
public:
    ...
    // ajouter de l'hexa en big
    size_t add_hexa_big(const std::string& hex, const std::string& key = "") {
        size_t offset = content.size();

        std::string h = hex;

        // si impair -> compléter devant
            if (h.size() & 1) {
            h.insert(h.begin(), '0');
        }

        // écrire tel quel (big endian)
        for (size_t i = 0; i < h.size(); i += 2) {
            uint8_t byte = hex_to_byte(h[i], h[i + 1]);
    ...
    //ajouter de l'hexa en little
    size_t add_hexa_little(const std::string& hex, const std::string& key = "") {
        size_t offset = content.size();
        ...
        // convertir en bytes temporaires
        std::vector<uint8_t> bytes;
        for (size_t i = 0; i < h.size(); i += 2) {
            bytes.push_back(hex_to_byte(h[i], h[i + 1]));
        }

        // écrire en little endian (inversion des octets)
...
public:
    ...
    bool test_code = false; //savoir si on doit tester le code
...
private:
    ...
    struct RelocPatch {
        std::string raw_code;
        size_t index_text;
        std::vector<u8> code_assembled;
    };

    std::vector<RelocPatch> patches;
    ...
    static inline bool is_label_line(const std::string& line) {
        // xxx:
    ...
    static inline bool starts_with_jump(const std::string& line) {
        ...
        for (auto& j : jumps) {
            if (t.size() >= j.size() && strncasecmp(t.c_str(), j.c_str(), j.size()) == 0)
                return true;
        }
        return false;
    }

    void test_asm_block(const std::string& func_name, const std::string& raw_code) {
        // 1) tentative globale
        try {
            std::string safe = resolve_symbols(raw_code, false);
            assemble(safe.c_str());
            return; // OK
        }
        catch (...) {
            // on va analyser ligne par ligne
        }
        ...
        while (std::getline(iss, line)) {
            ...
            std::string t = trim_copy(line);
            if (t.empty()) continue;
            if (is_label_line(t)) continue;
            if (starts_with_jump(t)) continue;

            std::string safe_line = resolve_symbols(t, false);

            try {
                assemble(safe_line.c_str());
            }
                catch (...) {
                std::cerr << "[ASM ERROR] function: " << func_name
                      << " | line: " << line_num
                      << " | content: " << t << std::endl;
                any_error = true;
            }
        ...
        if (!any_error) {
        std::cerr << "[ASM ERROR] function: " << func_name
                  << " | problem: jump/label resolution" << std::endl;
        }
    }
...
private:
    std::string resolve_symbols(const std::string& code, bool real, u32 offset = 0, std::vector<u8> assembled = {}) {
        std::string out;
        out.reserve(code.size());

        u64 pointor_assembled = 0;
        ...
        for (size_t i = 0; i < code.size(); ) {
            if (code[i] == '$') {
                ...
                if (real) {
                    ...
                    u64 index_line_assembly = 0;
                    if (!assembled.empty() && value != 0xFFFFFFFFFFFFFFFF) {
                    for (auto& patch : patches) {
                        if (patch.raw_code == code && patch.index_text == i) {
                            auto start_line_assembly = std::search(assembled.begin(),assembled.end(),patch.code_assembled.begin(),patch.code_assembled.end());
                            if (start_line_assembly != assembled.end()) {
                                auto end_line_assembly = start_line_assembly + patch.code_assembled.size();
                                u64 end_line_asm = static_cast<u64>(std::distance(assembled.begin(), end_line_assembly));
                                index_line_assembly = pointor_assembled + end_line_asm + (u64)offset + IMAGE_BASE;
                                assembled.erase(assembled.begin(), end_line_assembly);
                                pointor_assembled += end_line_asm;
                            }
                            break;
                        }
                    }
                    }
                    if (index_line_assembly && value != 0xFFFFFFFFFFFFFFFF) {
                        calc_value = (u32)((int32_t)(value-index_line_assembly));
                    }
                } else {
                    size_t j = i;
                    if (code[j]=='\n') j--;
                    ...
                    if (at_pos != std::string::npos) {
                        size_t symbol_start = at_pos + 1;
                        size_t symbol_end = symbol_start;
                        while (symbol_end < line.size() && (isalnum(line[symbol_end]) || line[symbol_end] == '_')) {
                            symbol_end++;
                        }
                        std::string symbol = line.substr(symbol_start, symbol_end - symbol_start);
                        line.replace(at_pos, symbol_end - at_pos, "0xFFFFFFFF");
                    }
                    ...
                    patch.raw_code = code;
                    try {
                        patch.code_assembled = assemble(line.c_str());
                    } catch (...) {
                        std::cerr << "[ASM ERROR] line : " << line << std::endl;
                    }
                    patches.push_back(patch);
                }

                out += std::to_string(calc_value);
            } else {
                out += code[i++];
            }
        }
...
public:
    void prepare(u32 text_rva) {
        ...
        content.clear();
        for (auto& f : functions) {
            std::string final_code = resolve_symbols(f.raw_code, true, base_rva + f.offset, f.assembled);
...
public:
    ...
    void set_cmd(bool rep) {
        is_console=rep;
    }
    //choisir si on veux dll ou exe
    void set_dll(bool rep) {
        is_dll = rep;
    }
    ...
    // ---- EDATA ----
    void edata_set_dll_name(const std::string& name = "mydll.dll") {
        return edata.set_dll_name(name);
    }
    void edata_add_export(const std::string& name, u32 rva = 0xFFFFFFFF, const std::string& localname = "") {
        return edata.add_export(name,rva,localname);
    }
    void edata_add_reexport(const std::string& name, const std::string& link) {
        return edata.add_reexport(name, link);
    }
    bool edata_have_unresolved_export_rva() {
        return edata.have_unresolved_export_rva();
    }
    ...
    size_t data_add_little(const std::string& hex, const std::string& key = "") {
        return data.add_hexa_little(hex,key);
    }
    size_t data_add_big(const std::string& hex, const std::string& key = "") {
        return data.add_hexa_big(hex,key);
    }
    ...
    u64 get_text_va(const std::string& name) {
        ...
        if (reponse == 0xFFFFFFFF) {
            if (see_exception_not_fatal) {
                std::cout << "la fonction " << name << " n'a pas été résolu ou n'existe pas !" << std::endl;
            }
            return 0;
        }
        return IMAGE_BASE + static_cast<u64>(reponse);
    ...
    void set_test_code(bool test_a_code) {
        asm_text.test_code=test_a_code;
    }
    ...
    void push_section(std::string type) {
        ...
        if (type == ".idata") {
            ...
            edata.prepare(0x0); // adresse inconnue pour l'instant

            if (edata.get_size()==0) {
                std::cout << "erreur : edata est vide !" << std::endl;
                return;
            }

            s.name = ".edata";
            s.content = edata.get_content();
            s.virtualAddress = 0x0;
            s.virtualSize = edata.get_size();
            s.sizeOfRawData = edata.get_size();
            s.characteristics = 0x40300040;
            ...
            //mettre a jour les rva de edata pour les fonctions exporter
            for (auto& edata_func : edata.exports) {
                edata_func.rva = asm_text.get_function_rva(edata_func.localname);
                if (edata_func.rva == 0xFFFFFFFF) {
                    //regarde les variables si pas dans les functions
                    u32 reponse = get_section_virtual_address(".data");
                    if (reponse == 0xFFFFFFFF) continue; //pas de section data
                    edata_func.rva = data.get_rva(edata_func.localname, reponse); //renvoie 0xFFFFFFFF si pas donc ok
                }
            }

            // refaire edata avec la vraie RVA
            edata.prepare(info.virtualAddress);
            ...
            // set data directory import
            pb.set_data_directory(IMAGE_DIRECTORY_ENTRY_EXPORT, info.virtualAddress, s.sizeOfRawData);
            ...
            s.virtualAddress = pb.get_section(this_index).virtualAddress;
    ...
    void update_section() {
        pb.layout();
        for (auto& section_ : sections) {
            if (section_.name == ".text") {
                push_section(section_.name);
            }
        }
        for (auto& section_ : sections) {
            if (section_.name == ".data" || section_.name == ".edata" || section_.name == ".idata" || section_.name == ".rsrc") {
                push_section(section_.name);
            }
        }
    }
    ...
    bool buid_exe() {
        if (!addr_entry_point || name_exe.empty()) {
            ...
            return false; //erreur paramètre nécessaire
        }

        vector<u8> pe = pb.build(size_t(addr_entry_point),is_console, is_dll);
        // Écrit le fichier
        HANDLE f = CreateFileA(name_exe.c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);

        if (f == INVALID_HANDLE_VALUE) {
            DWORD error = GetLastError();
            std::cerr << "Erreur : impossible de créer le fichier '" << name_exe << "' (code d'erreur : " << error << ")." << std::endl;
            switch (error) {
                case ERROR_ACCESS_DENIED:
                std::cerr << "Droits insuffisants pour écrire dans le répertoire." << std::endl;
                break;
                case ERROR_PATH_NOT_FOUND:
                std::cerr << "Chemin du fichier invalide." << std::endl;
                break;
                case ERROR_SHARING_VIOLATION:
                std::cerr << "Le fichier est déjà ouvert par un autre processus." << std::endl;
                break;
            }
            return false;
        }
        ...
        DWORD written;
        BOOL success = WriteFile(f, pe.data(), (DWORD)pe.size(), &written, NULL);
        CloseHandle(f);

        if (!success) {
            DWORD error = GetLastError();
            std::cerr << "Erreur : impossible d'écrire dans le fichier '" << name_exe << "' (code d'erreur : " << error << ")." << std::endl;
            return false;
        }

        if (written != pe.size()) {
            std::cerr << "Erreur : seulement " << written << " octets écrits sur " << pe.size() << " attendus." << std::endl;
            return false;
        }
